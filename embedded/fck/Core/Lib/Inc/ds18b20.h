...
#if (_DS18B20_USE_FREERTOS==1) // FreeRTOS 안쓰면
#include "cmsis_os.h"  // 이거 못쓰고
#define	Ds18b20Delay(x)			osDelay(x) // 디파인도 안되고
#else
#define	Ds18b20Delay(x)			HAL_Delay(x) // else가 됐으니 HAL_Delay를 사용하게 된다. Systick이 된다
#endif
