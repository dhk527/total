...
url_round = config['URL']['url_round']
...
# 포지션별 개인 승률 계산
individual_win_ratios = defaultdict(lambda: [0, 0])  # [승리 횟수, 총 경기 횟수]를 저장하는 defaultdict

for matches_info in match_sum_list:
    for puuid, position, win in matches_info:
        if puuid in our_team:
            if win:
                individual_win_ratios[(puuid, position)][0] += 1  # 승리 횟수 증가
            individual_win_ratios[(puuid, position)][1] += 1  # 총 경기 횟수 증가

# 각 포지션에서 가장 승률이 높은 팀원을 찾기
best_positions = {}
for position in positions:
	...
	if best_puuid:
		best_positions[position] = (best_puuid, best_win_rate)

print("sorting individual_win_ratios Name")
sorted_individual_win_ratios = sorted(individual_win_ratios.items(), key=lambda x:our_team[x[0][0]])
